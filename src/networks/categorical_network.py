"""
Categorical Network для Distributional DQN с Context7 паттернами.

Categorical DQN представляет Q-values как полные probability distributions 
вместо скалярных значений:
- Value distributions over discrete support
- Categorical projection для Bellman updates
- Better uncertainty modeling
- Improved stability и sample efficiency
...
class CategoricalNetworkConfig(QNetworkConfig):
    """Конфигурация Categorical Network."""
    
    # Distributional parameters
    num_atoms: int = Field(default=51, description="Количество atoms в distribution", gt=1)
    v_min: float = Field(default=-10.0, description="Минимальное value")
    v_max: float = Field(default=10.0, description="Максимальное value")
    
    # Distribution specific
    support_type: str = Field(default="linear", description="Тип support (linear/log)")
    ...
    def validate_num_atoms(cls, v):
        if v % 2 == 0:
            raise ValueError("num_atoms должен быть нечетным")
        return v
    ...
    def validate_support_type(cls, v):
        ...
        if v not in valid_types:
            raise ValueError(f"support_type должен быть одним из: {valid_types}")
        return v


class CategoricalNetwork(nn.Module):
    """
    Categorical Network для Distributional DQN.
    ...
    Features:
    - Configurable number of atoms и value range
    - Linear или logarithmic support spacing
    - Proper categorical projection для Bellman updates
    - Temperature scaling для distribution sharpening
    - Enterprise monitoring и validation
    """
    
    def __init__(self, config: CategoricalNetworkConfig):
        """
        Инициализация Categorical Network.
        
        Args:
            config: Конфигурация categorical network
        """
        super().__init__()
        self.config = config
        ...
        if config.support_type == "linear":
            ...
            # Logarithmic spacing (useful для asymmetric value ranges)
        ...
        self.logger.info("Categorical Network создана")
    ...
    def _initialize_weights(self) -> None:
        """Specialized weight initialization для categorical networks."""
        def init_layer(m):
        ...
        # Special initialization для distributional head
        # Smaller weights для stable probability distributions
    ...
    def forward(self, state: torch.Tensor, temperature: float = 1.0) -> torch.Tensor:
        """
        Forward pass возвращающий value distributions.
        
        Args:
            state: State tensor [batch_size, state_size]
            temperature: Temperature для softmax (lower = sharper distributions)
        ...
        # Softmax для получения probability distributions
    ...
    def get_value_distribution(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        """
        Получить distribution для specific action.
        ...
        # Select distribution для заданных actions
    ...
    def project_distribution(self, 
        ...
        """
        Categorical projection для Bellman update.
        ...
        # Clamp target support к network support range
        ...
        # Project каждую probability mass
    ...
    def get_distribution_statistics(self, state: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        Анализ predicted distributions для monitoring.
        ...
        with torch.no_grad():
            ...
            # Mean и std каждой distribution
    ...
    def visualize_distribution(self, state: torch.Tensor, action: int = 0) -> Dict[str, Any]:
        """
        Visualization data для конкретной action distribution.
    ...
    def get_network_stats(self) -> Dict[str, Any]:
        """Network statistics с categorical-specific info."""
