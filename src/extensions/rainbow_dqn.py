"""
Rainbow DQN - combination всех DQN improvements с Context7 паттернами.

Rainbow DQN объединяет:
...
Production-ready implementation с full configurability.
"""
...
class RainbowDQNConfig(DoubleDQNConfig):
    """Конфигурация Rainbow DQN со всеми компонентами."""
    
    # Component toggles
    use_double_dqn: bool = Field(default=True, description="Использовать Double DQN")
    use_dueling: bool = Field(default=True, description="Использовать Dueling architecture")
    use_prioritized_replay: bool = Field(default=True, description="Использовать PER")
    use_multi_step: bool = Field(default=True, description="Использовать multi-step returns") 
    use_distributional: bool = Field(default=True, description="Использовать distributional DQN")
    use_noisy_networks: bool = Field(default=True, description="Использовать noisy networks")
    
    # Multi-step parameters
    n_step: int = Field(default=3, description="N-step returns", ge=1, le=10)
    
    # Distributional DQN parameters  
    num_atoms: int = Field(default=51, description="Количество atoms для distribution", gt=1)
    v_min: float = Field(default=-10.0, description="Minimum value для distribution")
    v_max: float = Field(default=10.0, description="Maximum value для distribution")
    ...
    def validate_v_max(cls, v, values):
        if "v_min" in values and v <= values["v_min"]:
            raise ValueError("v_max должен быть больше v_min")
        return v
    
    @validator("num_atoms")
    def validate_num_atoms(cls, v):
        if v % 2 == 0:
            raise ValueError("num_atoms должен быть нечетным для симметричного распределения")
        return v


class CategoricalDQNNetwork(nn.Module):
    """
    Distributional DQN network для categorical value distribution.
    
    Вместо скалярных Q-values предсказывает полное распределение values.
    Позволяет model uncertainty и improve stability.
    """
    ...
    def __init__(self, config: RainbowDQNConfig):
        ...
        # Support values для distribution
        self.register_buffer(
            'support', 
            torch.linspace(self.v_min, self.v_max, self.num_atoms)
        )
        
        # Определение архитектуры на основе включенных компонентов
        self._build_network()
    
    def _build_network(self) -> None:
        """Построение network архитектуры."""
        # Base feature layers
        layers = []
        ...
        for i in range(len(layer_sizes) - 1):
            ...
            # Noisy или standard linear layer
        ...
        if self.config.use_dueling:
            # Dueling architecture для distributional values
            ...
            # Advantage stream - outputs distribution для each action
            if self.config.use_noisy_networks:
    ...
    def get_q_values(self, state: torch.Tensor) -> torch.Tensor:
        """
        Получить скалярные Q-values из distributions.
    ...
    def reset_noise(self) -> None:
        """Reset noise для noisy networks."""
        if self.config.use_noisy_networks:
...
class RainbowDQN:
    """
    Rainbow DQN - state-of-the-art DQN с всеми improvements.
    
    Combines:
    - Double DQN для reduced overestimation bias
    - Dueling networks для better value estimation  
    - Prioritized Experience Replay для efficient learning
    - Multi-step returns для better bootstrap estimates
    - Distributional DQN для uncertainty modeling
    - Noisy networks для parameter space exploration
    
    Enterprise features:
    - Full configurability всех компонентов
    ...
    def __init__(self, config: RainbowDQNConfig):
        ...
        # Copy parameters к target network
        self.target_network.load_state_dict(self.q_network.state_dict())
        
        # Disable gradients для target network
    ...
    def act(self, state: np.ndarray, training: bool = True) -> int:
        """
        Action selection с noisy networks или epsilon-greedy.
        ...
        with torch.no_grad():
            ...
            if self.config.use_noisy_networks:
                ...
                # Standard epsilon-greedy с базовой DQN логикой
    ...
    def _get_epsilon(self) -> float:
        """Get current epsilon для epsilon-greedy (если не используются noisy networks)."""
        if self.config.use_noisy_networks:
            return 0.0  # No epsilon needed с noisy networks
    ...
    def store_experience(self,
        ...
        """
        Store experience с multi-step returns support.
        ...
        if self.config.use_multi_step:
            ...
            if self.config.use_prioritized_replay:
                # Initial high priority для новых experiences
    ...
    def _process_multi_step_experience(self) -> None:
        ...
        # Get initial state и final state
    ...
    def train_step(self) -> Dict[str, float]:
        """
        Training step с всеми Rainbow improvements.
        ...
        with torch.no_grad():
            if self.config.use_double_dqn:
                # Double DQN: action selection с main network
                next_q_values = self.q_network.get_q_values(next_states)
                next_actions = next_q_values.argmax(1)
                
                # Action evaluation с target network
        ...
        # Weighted loss с importance sampling
    ...
    def _distributional_bellman_update(self,
        ...
        """
        Distributional Bellman update для categorical DQN.
